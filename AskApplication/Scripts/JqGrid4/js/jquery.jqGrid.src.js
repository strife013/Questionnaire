	...
	return this.each( function() {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				rowData.push("");
				if( ni ) {
				    rowData.push(addRowNum(0, i, ts.p.page, ts.p.rowNum,rid));
				}
				if( gi ) {
...
$.jgrid.extend({
	...
	remapColumns : function(permutation, updateCells, keepHeader)
    {
        ...
        var ts = this.get(0);
        //isUpdateCells判断排列表头还是排列数据
        function resortRows(parent, clobj, isUpdateCells) {
            $(">tr" + (clobj || ""), parent).each(function () {
                var row = this;
                var elems = $.makeArray(row.cells);
                var rowId = $(row).parent().parent()[0].id;
                //排列数据时跳过子表
                if (rowId == ts.id && isUpdateCells) {
		...
		resortRows($("thead:first", ts.grid.hDiv), keepHeader && ":not(.ui-jqgrid-labels)", false);
		if (updateCells) {
			resortRows($("#"+$.jgrid.jqID(ts.p.id)+" tbody:first"), ".jqgfirstrow, tr.jqgrow, tr.jqfoot", true);
		}
		if (ts.p.footerrow) {
